...
Tile labels use `--wp-tile-label-inset-inline` (12px) and
`--wp-tile-label-inset-bottom` (8px), independently of content padding.
Override these properties on a `Theme` or individual tile to customize the
insets. Labels remain in document flow so wrapped text and live-tile controls
continue to reserve space.
