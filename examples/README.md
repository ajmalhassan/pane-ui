...
The project gallery renders `shared/SettingsExample.tsx` and `shared/InboxExample.tsx`. Both are client components with no required props. Import `@pane-ui/react/styles.css` once in the host application before rendering them. Their extra layout styles are CSS modules.
